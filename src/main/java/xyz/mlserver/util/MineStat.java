...
{
    public static final String VERSION = "1.0.0";         // MineStat version
        ...
        switch(requestType) {
            ...
            default:
                ...
                if(port == DEFAULT_BEDROCK_PORT) {
                    ...
                    if(retval == Retval.SUCCESS) break;
                }
                // SLP 1.4/1.5
                retval = legacyRequest(address, port, getTimeout());
                // SLP 1.8b/1.3
                if(retval != Retval.SUCCESS && retval != Retval.CONNFAIL) retval = betaRequest(address, port, getTimeout());
                // SLP 1.6
                if(retval != Retval.CONNFAIL) retval = extendedLegacyRequest(address, port, getTimeout());
                // SLP 1.7
                if(retval != Retval.CONNFAIL) retval = jsonRequest(address, port, getTimeout());
                // Bedrock/Pocket Edition
                if(retval != Retval.CONNFAIL && !bedrockAttempted) {
                    if(!isPortDefined) setPort(DEFAULT_BEDROCK_PORT);
                    retval = bedrockRequest(address, port, getTimeout());
                }
        }
    }
